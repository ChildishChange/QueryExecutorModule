...
public final class ArgumentParser {
    ...
    public static void parseInput(String[] args) {
        ...
        Option cacheEnabled = OptionBuilder.withArgName("Enable cache")
            .withLongOpt("enableCache")
            .isRequired(false)
            .create("cache");
        options.addOption(cacheEnabled);

        Option cacheEnabledLegacy = OptionBuilder.withArgName("Enable legacy cache")
            .withLongOpt("enableCacheLegacy")
            .isRequired(false)
            .create("cachelegacy");
        options.addOption(cacheEnabledLegacy);

        Option serverMode = OptionBuilder.withArgName("Server mode")
            .withLongOpt("serverMode")
            .isRequired(false)
            .create("server");
        options.addOption(serverMode);

        Option rootdir = OptionBuilder.withArgName("Root dir")
            .hasArg()
            .withLongOpt("rootdir")
            .isRequired(true)
            .create("root");
        ...
        Option optimize = OptionBuilder.withArgName("Enable optmizations")
            .withLongOpt("enableOptimize")
            .isRequired(false)
            .create("optimize");
        ...
        if (cmd.hasOption("enableCache")) {
            Const.cacheEnable_$eq(true);
        }

        if (cmd.hasOption("enableCacheLegacy")) {
            Const.cacheEnableLegacy_$eq(true);
        }

        if (cmd.hasOption("serverMode")) {
            Const.serverMode_$eq(true);
        }

        if (cmd.hasOption("rootdir")) {
            Const.rootDir_$eq(cmd.getOptionValue("rootdir"));
        }

        if (cmd.hasOption("optimize")) {
            Const.optimize_$eq(true);
        }
