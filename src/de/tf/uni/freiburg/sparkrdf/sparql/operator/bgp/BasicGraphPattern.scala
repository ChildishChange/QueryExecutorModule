...
trait BasicGraphPattern {
    ...
    protected def matchBGP(triples: Broadcast[java.util.List[TriplePattern]],
        ...
        val adjacentMessages = AdjacentResultsExchanger
            .calculateResultsMessages(g.vertices, triples)
        ...
        while (activeVertices) {
            ...
            activeVertices = newVerts.mapPartitions(iter => Iterator(iter.hasNext), true)
                .reduce(_ && _)
         ...
         */
        val partitions = Math.max(matching.partitions.length / 5, 108)

        val endTime = System.currentTimeMillis()
//        printf("BGP Time: %d, %d, %d, %d\n",
//               step1Time - startTime,
//               step2Time - step1Time,
//               step3Time - step2Time,
//               endTime - step3Time)

        if (partitions < matching.partitions.length) {
    ...
    protected def matchBGPFast(triples: Broadcast[java.util.List[TriplePattern]],
                           graph: Graph[VertexInterface, EdgeAttribute],
                           context: SparkContext): RDD[(VertexId, VertexInterface)] = {
        val startTime = System.currentTimeMillis()
