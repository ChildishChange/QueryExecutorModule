        ...
        val messages = vertices.flatMap(idNode => {
            ...
            if (node.verifyNodeMatches(triples.value, false)) {
                ...
                if (node.hasParentNodes()) {
                    ...
                    node.getParentIDs().asScala.map(Long2long).foreach(parent => {
                        ...
                        if (node.hasResults()) {
                            ...
                            sendToParents
                                .addChildResults(node.getSendResults(parent, triples.value))
                        } else {
                            /*
                ...
                if (node.hasChildNodes()) {
                    // Add all messages for parents
                    node.getChildIDs().asScala.map(Long2long).foreach(child => {
                        ...
                        if (node.hasResults()) {
                            ...
                            sendToChildren
                                .addParentResults(node.getSendResults(child, triples.value))
                        } else {
                            /*
    ...
    def joinVerifiedResultsIntoNodes(messages: RDD[(VertexId, IMessage)],
        ...
        val newVerts = graph.vertices.innerJoin(reducedMessages)((vertexId, node, message) => {
            ...
            if (message.hasRemoveAttributes()) {
                message.getRemoveAttributes().asScala
                    .foreach(attr => node.removeAdjacentMatches(attr))
            }
