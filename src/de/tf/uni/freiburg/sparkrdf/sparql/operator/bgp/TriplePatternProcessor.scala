...
object TriplePatternProcessor {
    ...
    def matchBGP(triples: Broadcast[java.util.List[TriplePattern]],
        ...
        graph.triplets.flatMap(triplet => {
            ...
            triples.value.asScala.foreach(queryTriple => {
                // Check if the TriplePattern is fulfilled
                if (queryTriple
                    .isFulfilledByTriplet(subjectNode, edge.getEdgeAttribute(), objectNode)) {
                    ...
                    val subkey: CompositeKey = new CompositeKey(
                        queryTriple.getStringRepresentation(), Position.SUBJECT)
                    val subjectResult: NodeResultMessage = new NodeResultMessage(subkey,
                                                                                 subResultValue);
                    ...
                    if (!TriplePatternUtils
                        .isObjectLiteral(triples.value, queryTriple.getObject())) {
                        val objResultValue: ResultValue = new ResultValue(edge.getEdgeAttribute(),
                                                                          subjectNode
                                                                              .getAttribute());
                        val objKey: CompositeKey = new CompositeKey(
                            queryTriple.getStringRepresentation(), Position.OBJECT)
                        val objectResult: NodeResultMessage = new NodeResultMessage(objKey,
                                                                                    objResultValue);
                    ...
                    if (queryTriple.isPredicateVariable() && edge.hasNodeId()) {
                         ...
                         */
                        val predResultValue: ResultValue = new ResultValue(
                            subjectNode.getAttribute(), objectNode.getAttribute());
                        val predKey: CompositeKey = new CompositeKey(
                            queryTriple.getStringRepresentation(), Position.PREDICATE)
    ...
    def joinNeighborsIntoGraph(msgs: RDD[(VertexId, IMessage)],
        ...
        val newVerts = graph.vertices.innerJoin(adjacentNodes)((vertexId, node, message) => {
            ...
            msg.getAdjacentNodes().asScala.foreach(adjNode => {
