...
object InitialSolutionMappingBuilder {
    ...
    def findVertexResultNodes(toSearch: java.util.Set[TriplePattern],
        ...
        vertices.flatMap(vertex => {
            ...
            results.asScala.foreach(resultPair => {
                ...
                if (key.getPosition().equals(Position.SUBJECT)) {
                    ...
                    toSearch.asScala.foreach(triple => {

                        if (key.getTriplePattern().equals(triple.getStringRepresentation())) {

                            // Add all results of this node
                            resultPair._2.asScala.foreach(resultValue => {

                                // Add all variable fields to the solution mapping
                                val solutionMapping: SolutionMapping = new SolutionMapping(
                                    key.getTriplePattern())
                                if (TriplePatternUtils.isVariable(key.getSubject())) {
                                    solutionMapping.addMapping(key.getSubject(), vertex._2
                                        .asInstanceOf[VertexAttribute].getAttribute())
                                }

                                if (TriplePatternUtils.isVariable(key.getPredicate())) {
                                    solutionMapping.addMapping(key.getPredicate(),
                                                               resultValue.getEdgeAttribute())
                                }

                                if (TriplePatternUtils.isVariable(key.getObject())) {
                                    solutionMapping.addMapping(key.getObject(), resultValue
                                        .getConnectedNodeAttribute())
                                }
