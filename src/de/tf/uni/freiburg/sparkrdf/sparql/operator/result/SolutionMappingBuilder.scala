...
/**
  ...
  */
object SolutionMappingBuilder extends Serializable {
    ...
    def joinSolutionMappings(partialResults: java.util.List[PartialResult],
        ...
        partialResults.asScala.map(partialResult => {

            var firstRound = true
            var solutionMappingForPartialResult: RDD[SolutionMapping] = null

            partialResult.getAllTriples.asScala.foreach(triplePattern => {
                // First round
                if (firstRound) {
                    ...
                    if (!isEmpty) {
                        ...
                        val joinVariables = TriplePatternUtils
                            .getJoinVariables(firstSolutionMapping.getStoredVariables(),
                                              triplePattern)
    ...
    private def getJoinKey(joinVariables: java.util.List[String],
        ...
        var joinKey: String = ""
        joinVariables.asScala.foreach(joinVariable => {
